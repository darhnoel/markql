...
template <typename T>
size_t bounded_vector_size(const T* vector,
                           size_t max_count,
...
flatbuffers::Offset<qastfb::Operand> build_operand_fb(flatbuffers::FlatBufferBuilder& builder,
  ...
  return qastfb::CreateOperand(builder,
                               static_cast<qastfb::Axis>(to_u8(operand.axis)),
...
flatbuffers::Offset<qastfb::ScalarExpr> build_scalar_expr_fb(flatbuffers::FlatBufferBuilder& builder,
...
flatbuffers::Offset<qastfb::FlattenExtractExpr> build_flatten_extract_expr_fb(
  ...
  for (const auto& condition : expr.case_when_conditions) conditions.push_back(build_expr_fb(builder, condition));

  std::vector<flatbuffers::Offset<qastfb::FlattenExtractExpr>> values;
  values.reserve(expr.case_when_values.size());
  for (const auto& value : expr.case_when_values) values.push_back(build_flatten_extract_expr_fb(builder, value));

  return qastfb::CreateFlattenExtractExpr(
      builder,
      static_cast<qastfb::FlattenExtractKind>(to_u8(expr.kind)),
      ...
      expr.selector_index.value_or(0),
      expr.selector_index.has_value(),
      expr.selector_last,
      args.empty() ? 0 : builder.CreateVector(args),
      builder.CreateString(expr.function_name),
      builder.CreateString(expr.string_value),
      expr.number_value,
      builder.CreateString(expr.alias_ref),
...
flatbuffers::Offset<qastfb::ScalarExpr> build_scalar_expr_fb(flatbuffers::FlatBufferBuilder& builder,
  ...
  return qastfb::CreateScalarExpr(builder,
                                  static_cast<qastfb::ScalarExprKind>(to_u8(expr.kind)),
                                  build_operand_fb(builder, expr.operand),
                                  builder.CreateString(expr.string_value),
                                  expr.number_value,
                                  builder.CreateString(expr.function_name),
...
flatbuffers::Offset<qastfb::CompareExpr> build_compare_expr_fb(flatbuffers::FlatBufferBuilder& builder,
  ...
  for (const auto& rhs : expr.rhs_expr_list) rhs_expr_list.push_back(build_scalar_expr_fb(builder, rhs));

  return qastfb::CreateCompareExpr(
      builder,
      static_cast<qastfb::CompareOp>(to_u8(expr.op)),
      build_operand_fb(builder, expr.lhs),
...
flatbuffers::Offset<qastfb::Expr> build_expr_fb(flatbuffers::FlatBufferBuilder& builder,
  ...
  if (std::holds_alternative<CompareExpr>(expr)) {
    return qastfb::CreateExpr(builder,
                              qastfb::ExprVariant_Compare,
                              build_compare_expr_fb(builder, std::get<CompareExpr>(expr)),
                              0,
                              0);
  }
  if (std::holds_alternative<std::shared_ptr<ExistsExpr>>(expr)) {
    ...
    return qastfb::CreateExpr(
        builder,
        qastfb::ExprVariant_Exists,
        0,
        qastfb::CreateExistsExpr(builder,
                                 static_cast<qastfb::Axis>(to_u8(exists.axis)),
                                 exists.where.has_value() ? build_expr_fb(builder, *exists.where) : 0),
        0);
  }
  const auto& binary = *std::get<std::shared_ptr<BinaryExpr>>(expr);
  return qastfb::CreateExpr(builder,
                            qastfb::ExprVariant_Binary,
                            0,
                            0,
                            qastfb::CreateBinaryExpr(builder,
                                                     static_cast<qastfb::BinaryOp>(to_u8(binary.op)),
...
flatbuffers::Offset<qastfb::Source> build_source_fb(flatbuffers::FlatBufferBuilder& builder,
  ...
  return qastfb::CreateSource(
      builder,
      static_cast<qastfb::SourceKind>(to_u8(source.kind)),
...
flatbuffers::Offset<qastfb::SelectItem> build_select_item_fb(flatbuffers::FlatBufferBuilder& builder,
                                                             const Query::SelectItem& item) {
  ...
  for (const auto& alias : item.flatten_aliases) flatten_aliases.push_back(builder.CreateString(alias));
  ...
  for (const auto& alias : item.flatten_extract_aliases) flatten_extract_aliases.push_back(builder.CreateString(alias));
  ...
  for (const auto& expr : item.flatten_extract_exprs) flatten_exprs.push_back(build_flatten_extract_expr_fb(builder, expr));

  return qastfb::CreateSelectItem(
      builder,
      static_cast<qastfb::Aggregate>(to_u8(item.aggregate)),
      static_cast<qastfb::TfidfStopwords>(to_u8(item.tfidf_stopwords)),
      builder.CreateString(item.tag),
      tfidf_tags.empty() ? 0 : builder.CreateVector(tfidf_tags),
      item.field.has_value() ? builder.CreateString(*item.field) : 0,
      item.tfidf_all_tags,
      item.tfidf_top_terms,
      item.tfidf_min_df,
      item.tfidf_max_df,
      item.inner_html_depth.value_or(0),
      item.inner_html_depth.has_value(),
      item.inner_html_auto_depth,
      ...
      item.flatten_depth.value_or(0),
      item.flatten_depth.has_value(),
      ...
      item.project_expr.has_value() ? build_flatten_extract_expr_fb(builder, *item.project_expr) : 0);
}
...
flatbuffers::Offset<qastfb::Query> build_query_fb(flatbuffers::FlatBufferBuilder& builder,
  ...
  if (query.with.has_value()) {
    ...
    for (const auto& cte : query.with->ctes) {
      ...
      ctes.push_back(qastfb::CreateWithCte(
          builder, builder.CreateString(cte.name), build_query_fb(builder, *cte.query)));
    }
  ...
  for (const auto& item : query.select_items) select_items.push_back(build_select_item_fb(builder, item));
  ...
  for (const auto& join : query.joins) {
    joins.push_back(qastfb::CreateJoinItem(
        builder,
        static_cast<qastfb::JoinType>(to_u8(join.type)),
        build_source_fb(builder, join.right_source),
        join.on.has_value() ? build_expr_fb(builder, *join.on) : 0,
        join.lateral));
  }
  ...
  for (const auto& order : query.order_by) {
    order_by.push_back(qastfb::CreateOrderBy(builder, builder.CreateString(order.field), order.descending));
  }

  std::vector<flatbuffers::Offset<flatbuffers::String>> exclude_fields;
  exclude_fields.reserve(query.exclude_fields.size());
  for (const auto& field : query.exclude_fields) exclude_fields.push_back(builder.CreateString(field));

  flatbuffers::Offset<qastfb::ExportSink> export_sink = 0;
  if (query.export_sink.has_value()) {
    export_sink = qastfb::CreateExportSink(
        builder,
        static_cast<qastfb::ExportSinkKind>(to_u8(query.export_sink->kind)),
        builder.CreateString(query.export_sink->path));
  }

  const auto table_options = qastfb::CreateTableOptions(
      builder,
      static_cast<qastfb::TrimEmptyCols>(to_u8(query.table_options.trim_empty_cols)),
      ...
      query.table_options.header_normalize,
      query.table_options.header_normalize_explicit);

  return qastfb::CreateQuery(
      builder,
      static_cast<qastfb::QueryKind>(to_u8(query.kind)),
      with_clause,
      select_items.empty() ? 0 : builder.CreateVector(select_items),
      build_source_fb(builder, query.source),
      joins.empty() ? 0 : builder.CreateVector(joins),
      query.where.has_value() ? build_expr_fb(builder, *query.where) : 0,
      order_by.empty() ? 0 : builder.CreateVector(order_by),
      exclude_fields.empty() ? 0 : builder.CreateVector(exclude_fields),
      query.limit.value_or(0),
      query.limit.has_value(),
      query.to_list,
      query.to_table,
      query.table_has_header,
      table_options,
      export_sink);
}
...
Operand parse_operand_fb(const qastfb::Operand* operand_fb) {
  ...
  Operand operand;
  operand.axis = enum_from_u8(static_cast<uint8_t>(operand_fb->axis()),
                              Operand::Axis::Descendant,
                              "Corrupted artifact: invalid operand axis");
  operand.field_kind = enum_from_u8(static_cast<uint8_t>(operand_fb->field_kind()),
  ...
  if (operand_fb->qualifier() != nullptr) operand.qualifier = read_fb_string(operand_fb->qualifier());
  return operand;
}
...
Query::SelectItem::FlattenExtractExpr parse_flatten_extract_expr_fb(
    const qastfb::FlattenExtractExpr* expr_fb,
    size_t depth = 0) {
  ...
  if (const auto* args_fb = expr_fb->args()) {
    const size_t count = bounded_vector_size(
        args_fb, kMaxCollectionCount, "Artifact limit exceeded: flatten args are too large");
  ...
  if (const auto* conditions_fb = expr_fb->case_when_conditions()) {
    ...
    for (size_t i = 0; i < count; ++i) expr.case_when_conditions.push_back(parse_expr_fb(conditions_fb->Get(i), depth));
  }
...
ScalarExpr parse_scalar_expr_fb(const qastfb::ScalarExpr* expr_fb, size_t depth) {
  ...
  if (const auto* args_fb = expr_fb->args()) {
    const size_t count = bounded_vector_size(
        args_fb, kMaxCollectionCount, "Artifact limit exceeded: scalar args are too large");
    expr.args.reserve(count);
    for (size_t i = 0; i < count; ++i) expr.args.push_back(parse_scalar_expr_fb(args_fb->Get(i), next_depth));
  }
  return expr;
}
  ...
  switch (expr_fb->variant()) {
    case qastfb::ExprVariant_Compare: {
      ...
      if (const auto* values_fb = expr_fb->compare()->rhs_values()) {
        ...
        for (size_t i = 0; i < count; ++i) expr.rhs.values.push_back(read_fb_string(values_fb->Get(i)));
      }
      if (expr_fb->compare()->lhs_expr() != nullptr) expr.lhs_expr = parse_scalar_expr_fb(expr_fb->compare()->lhs_expr());
      if (expr_fb->compare()->rhs_expr() != nullptr) expr.rhs_expr = parse_scalar_expr_fb(expr_fb->compare()->rhs_expr());
      if (const auto* rhs_fb = expr_fb->compare()->rhs_expr_list()) {
        ...
        for (size_t i = 0; i < count; ++i) expr.rhs_expr_list.push_back(parse_scalar_expr_fb(rhs_fb->Get(i)));
      }
      return expr;
    }
    case qastfb::ExprVariant_Exists: {
      ...
      ExistsExpr expr;
      expr.axis = enum_from_u8(static_cast<uint8_t>(expr_fb->exists()->axis()),
                               Operand::Axis::Descendant,
                               "Corrupted artifact: invalid exists axis");
      if (expr_fb->exists()->where_expr() != nullptr) expr.where = parse_expr_fb(expr_fb->exists()->where_expr(), next_depth);
    ...
    case qastfb::ExprVariant_Binary: {
      ...
      BinaryExpr expr;
      expr.op = enum_from_u8(static_cast<uint8_t>(expr_fb->binary()->op()),
                             BinaryExpr::Op::Or,
...
Source parse_source_fb(const qastfb::Source* source_fb, size_t query_depth) {
  ...
  Source source;
  source.kind = enum_from_u8(static_cast<uint8_t>(source_fb->kind()),
  ...
  if (source_fb->fragments_query() != nullptr) {
    source.fragments_query = std::make_shared<Query>(parse_query_fb(source_fb->fragments_query(), query_depth));
  }
  if (source_fb->fragments_raw() != nullptr) source.fragments_raw = read_fb_string(source_fb->fragments_raw());
  if (source_fb->parse_query() != nullptr) {
    source.parse_query = std::make_shared<Query>(parse_query_fb(source_fb->parse_query(), query_depth));
  }
  if (source_fb->parse_expr() != nullptr) source.parse_expr = std::make_shared<ScalarExpr>(parse_scalar_expr_fb(source_fb->parse_expr()));
  if (source_fb->derived_query() != nullptr) {
    source.derived_query = std::make_shared<Query>(parse_query_fb(source_fb->derived_query(), query_depth));
  }
  return source;
}
...
Query::SelectItem parse_select_item_fb(const qastfb::SelectItem* item_fb) {
  ...
  item.aggregate = enum_from_u8(static_cast<uint8_t>(item_fb->aggregate()),
  ...
  if (const auto* tags_fb = item_fb->tfidf_tags()) {
    const size_t count = bounded_vector_size(
        tags_fb, kMaxCollectionCount, "Artifact limit exceeded: string list is too large");
  ...
  item.tfidf_top_terms = read_fb_size(item_fb->tfidf_top_terms(), "Corrupted artifact: size value overflow");
  item.tfidf_min_df = read_fb_size(item_fb->tfidf_min_df(), "Corrupted artifact: size value overflow");
  item.tfidf_max_df = read_fb_size(item_fb->tfidf_max_df(), "Corrupted artifact: size value overflow");
  if (item_fb->has_inner_html_depth()) {
    item.inner_html_depth = read_fb_size(item_fb->inner_html_depth(), "Corrupted artifact: size value overflow");
  }
  ...
  if (item_fb->has_flatten_depth()) {
    item.flatten_depth = read_fb_size(item_fb->flatten_depth(), "Corrupted artifact: size value overflow");
  }
  if (const auto* aliases_fb = item_fb->flatten_aliases()) {
    const size_t count = bounded_vector_size(
        aliases_fb, kMaxCollectionCount, "Artifact limit exceeded: string list is too large");
    item.flatten_aliases.reserve(count);
    for (size_t i = 0; i < count; ++i) item.flatten_aliases.push_back(read_fb_string(aliases_fb->Get(i)));
  }
  if (const auto* aliases_fb = item_fb->flatten_extract_aliases()) {
    const size_t count = bounded_vector_size(
        aliases_fb, kMaxCollectionCount, "Artifact limit exceeded: string list is too large");
    item.flatten_extract_aliases.reserve(count);
    for (size_t i = 0; i < count; ++i) item.flatten_extract_aliases.push_back(read_fb_string(aliases_fb->Get(i)));
  }
  if (const auto* exprs_fb = item_fb->flatten_extract_exprs()) {
    ...
    for (size_t i = 0; i < count; ++i) item.flatten_extract_exprs.push_back(parse_flatten_extract_expr_fb(exprs_fb->Get(i)));
  }
  if (item_fb->expr() != nullptr) item.expr = parse_scalar_expr_fb(item_fb->expr());
  if (item_fb->project_expr() != nullptr) item.project_expr = parse_flatten_extract_expr_fb(item_fb->project_expr());
  return item;
}
...
Query parse_query_fb(const qastfb::Query* query_fb, size_t depth) {
  ...
  Query query;
  query.kind = enum_from_u8(static_cast<uint8_t>(query_fb->kind()),
  ...
  if (query_fb->with_clause() != nullptr) {
    ...
    const size_t cte_count = bounded_vector_size(
        ctes_fb, kMaxCollectionCount, "Artifact limit exceeded: CTE list is too large");
  ...
  if (const auto* select_fb = query_fb->select_items()) {
    const size_t count = bounded_vector_size(
        select_fb, kMaxCollectionCount, "Artifact limit exceeded: select list is too large");
    query.select_items.reserve(count);
    for (size_t i = 0; i < count; ++i) query.select_items.push_back(parse_select_item_fb(select_fb->Get(i)));
  }
  query.source = parse_source_fb(query_fb->source(), next_depth);
  if (const auto* joins_fb = query_fb->joins()) {
    const size_t count = bounded_vector_size(
        joins_fb, kMaxCollectionCount, "Artifact limit exceeded: join list is too large");
    query.joins.reserve(count);
    for (size_t i = 0; i < count; ++i) {
      const auto* join_fb = joins_fb->Get(i);
      ensure(join_fb != nullptr, "Corrupted artifact: missing join");
      Query::JoinItem join;
      join.type = enum_from_u8(static_cast<uint8_t>(join_fb->type()),
  ...
  if (query_fb->where_expr() != nullptr) query.where = parse_expr_fb(query_fb->where_expr(), next_depth);
  if (const auto* order_fb = query_fb->order_by()) {
    const size_t count = bounded_vector_size(
        order_fb, kMaxCollectionCount, "Artifact limit exceeded: order-by list is too large");
  ...
  if (const auto* exclude_fb = query_fb->exclude_fields()) {
    const size_t count = bounded_vector_size(
        exclude_fb, kMaxCollectionCount, "Artifact limit exceeded: string list is too large");
    query.exclude_fields.reserve(count);
    for (size_t i = 0; i < count; ++i) query.exclude_fields.push_back(read_fb_string(exclude_fb->Get(i)));
  }
  if (query_fb->has_limit()) query.limit = read_fb_size(query_fb->limit(), "Corrupted artifact: size value overflow");
  ...
  query.table_options.trim_empty_cols = enum_from_u8(
      static_cast<uint8_t>(query_fb->table_options()->trim_empty_cols()),
  ...
  query.table_options.stop_after_empty_rows = read_fb_size(query_fb->table_options()->stop_after_empty_rows(),
                                                           "Corrupted artifact: size value overflow");
  query.table_options.header_normalize = query_fb->table_options()->header_normalize();
  query.table_options.header_normalize_explicit = query_fb->table_options()->header_normalize_explicit();
...
std::string build_prepared_query_payload_flatbuffers(const Query& query) {
  ...
  qastfb::FinishPreparedQueryPayloadBuffer(
      builder, qastfb::CreatePreparedQueryPayload(builder, query_root));

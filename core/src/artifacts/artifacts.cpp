...
void write_query(BinaryWriter& writer, const Query& query);
Query read_query(BinaryReader& reader, size_t depth = 0);
...
void write_source(BinaryWriter& writer, const Source& source) {
  ...
  if (source.fragments_query != nullptr) write_query(writer, *source.fragments_query);
  writer.write_optional_string(source.fragments_raw);
  writer.write_bool(source.parse_query != nullptr);
  if (source.parse_query != nullptr) write_query(writer, *source.parse_query);
  ...
  if (source.derived_query != nullptr) write_query(writer, *source.derived_query);
}
...
Source read_source(BinaryReader& reader, size_t query_depth) {
  ...
  if (reader.read_bool()) {
    source.fragments_query = std::make_shared<Query>(read_query(reader, query_depth));
  }
  source.fragments_raw = reader.read_optional_string();
  if (reader.read_bool()) source.parse_query = std::make_shared<Query>(read_query(reader, query_depth));
  if (reader.read_bool()) source.parse_expr = std::make_shared<ScalarExpr>(read_scalar_expr(reader));
  if (reader.read_bool()) source.derived_query = std::make_shared<Query>(read_query(reader, query_depth));
  return source;
}
...
Query read_query(BinaryReader& reader, size_t depth) {
  const size_t next_depth = checked_next_depth(
  ...
  if (reader.read_bool()) {
    ...
    for (size_t i = 0; i < cte_count; ++i) {
      ...
      cte.query = std::make_shared<Query>(read_query(reader, next_depth));
...
std::string build_prepared_meta_payload(const Query& query,
...
Query parse_prepared_query_payload(const std::string& payload) {
  BinaryReader reader(payload);
  Query query = read_query(reader);
...
void write_prepared_query_artifact_file(const Query& query,
                                        ...
                                        const std::string& path) {
  std::vector<detail::SectionView> sections;
  sections.push_back(detail::SectionView{"META", build_prepared_meta_payload(query, original_query)});
  sections.push_back(detail::SectionView{"QAST", build_prepared_query_payload(query)});
  detail::write_file_bytes(path,
...
PreparedQueryArtifact prepare_query_artifact(const std::string& query_text) {
  ...
  artifact.info.header.required_features = 0;
...
PreparedQueryArtifact read_prepared_query_artifact_file(const std::string& path) {
  ...
  artifact.query = parse_prepared_query_payload(query_payload->payload);

...
QueryResult execute_meta_query(const Query& query, const std::string& source_uri) {
  switch (query.kind) {
    case Query::Kind::ShowInput: {
      return build_meta_result({"key", "value"}, {{"source_uri", source_uri}});
    }
    case Query::Kind::ShowInputs: {
      return build_meta_result({"source_uri"}, {{source_uri}});
    }
    case Query::Kind::ShowFunctions: {
      return build_meta_result(
          {"function", "returns", "description"},
          {
              ...
              {"direct_text(tag|self)", "string",
               "Immediate text content of a tag or current row node"},
              {"first_text(tag WHERE ...)", "string",
               "First scoped TEXT match (alias of TEXT(..., 1))"},
              ...
              {"regex_replace(str, pattern, repl)", "string", "Replace regex matches (ECMAScript)"},
              ...
              {"inner_html(tag|self[, depth|MAX_DEPTH])", "string",
               "Minified HTML inside a tag/current row node"},
              {"raw_inner_html(tag|self[, depth|MAX_DEPTH])", "string",
               "Raw inner HTML without minification"},
              {"flatten_text(tag[, depth])", "string[]",
               "Flatten descendant text at depth into columns"},
    ...
    case Query::Kind::ShowAxes: {
      return build_meta_result({"axis", "description"}, {
                                                            {"parent", "Parent node"},
    ...
    case Query::Kind::ShowOperators: {
      return build_meta_result({"operator", "description"},
                               {
                                   {"=", "Equality"},
                                   {"<>", "Not equal"},
                                   {"<, <=, >, >=", "Ordered comparison"},
    ...
    case Query::Kind::DescribeDoc: {
      return build_meta_result({"column_name", "type", "nullable", "notes"},
                               {
      ...
      return build_meta_result(
          {"category", "name", "syntax", "notes"},
          {
              ...
              {"clause", "ORDER BY", "ORDER BY <field> [ASC|DESC]",
               "node_id, tag, text, parent_id, sibling_pos, max_depth, doc_order; SUMMARIZE uses "
               "tag/count"},
              ...
              {"output", "TO TABLE",
               ...
               "[, EMPTY_IS=...][, STOP_AFTER_EMPTY_ROWS=n][, FORMAT=SPARSE][, "
               "SPARSE_SHAPE=LONG|WIDE]"
              ...
              {"output", "TO NDJSON", "TO NDJSON(['file.ndjson'])",
               "Export rows as newline-delimited JSON"},
              ...
              {"source", "fragments_query", "FRAGMENTS(SELECT inner_html(...) FROM doc)",
               "Subquery returns HTML strings"},
              ...
              {"function", "trim", "trim(text(...)) | trim(inner_html(...))", "Trim whitespace"},
              ...
              {"function", "regex_replace", "regex_replace(str, pattern, repl)",
               "Regex replacement (ECMAScript); NULL on invalid pattern"},
              {"function", "case expression", "CASE WHEN <expr> THEN <value> [ELSE <value>] END",
              ...
              {"function", "first_attr", "first_attr(tag, attr WHERE ...)",
               "First scoped attr match"},
              {"function", "last_attr", "last_attr(tag, attr WHERE ...)", "Last scoped attr match"},
              {"function", "project", "project(tag) AS (alias: expr, ...)",
               "Expressions: TEXT/ATTR/DIRECT_TEXT/COALESCE plus SQL string functions"},
              {"function", "flatten_extract", "flatten_extract(tag) AS (alias: expr, ...)",
               "Expressions: TEXT/ATTR/DIRECT_TEXT/COALESCE plus SQL string functions"},

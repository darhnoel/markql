...
void test_count_table_rows_header() {
  markql::QueryResult::TableResult table;
  table.rows = {
      {"H1", "H2"},
      {"a", "b"},
      {"c", "d"},
  };
  size_t count = markql::cli::count_table_rows(table, true);
...
void test_count_table_rows_no_header() {
  ...
  size_t count = markql::cli::count_table_rows(table, false);
...
void test_column_width_cjk_wide() {
  std::string text =
      ...
      "a";
  size_t width = markql::cli::column_width(text, 0, text.size());
...
void test_column_to_index_cjk_boundary() {
  std::string text =
      "\xE6\x97\xA5\xE6\x9C\xAC"
      "a";
  size_t idx = markql::cli::column_to_index(text, 0, text.size(), 4);
...
void test_inspect_sql_input_line_comment_only() {
  auto inspection = markql::cli::inspect_sql_input("-- just comment");
...
void test_inspect_sql_input_unterminated_block_comment() {
  auto inspection = markql::cli::inspect_sql_input("/* missing");
...
void test_parse_query_source_doc_alias_targets_doc_input() {
  auto parsed = markql::cli::parse_query_source("SELECT n.node_id FROM doc AS n");
...
void test_parse_query_source_cte_alias_not_dispatch_alias() {
  auto parsed = markql::cli::parse_query_source(
...
void test_parse_query_source_named_input_alias_dispatches() {
  auto parsed = markql::cli::parse_query_source("SELECT x.node_id FROM x WHERE x.tag = 'li'");
...
void register_cli_utils_tests(std::vector<TestCase>& tests) {
  ...
  tests.push_back(
      {"proportional_column_end_maps_to_end", test_proportional_column_end_maps_to_end});
  tests.push_back({"proportional_column_scales_middle", test_proportional_column_scales_middle});
  tests.push_back(
      {"proportional_column_zero_source_len", test_proportional_column_zero_source_len});
  ...
  tests.push_back(
      {"inspect_sql_input_line_comment_only", test_inspect_sql_input_line_comment_only});
  tests.push_back(
      {"inspect_sql_input_block_comment_only", test_inspect_sql_input_block_comment_only});

...
void test_parse_parse_source_forms() {
  auto parsed_literal = markql::parse_query("SELECT li FROM PARSE('<ul><li>1</li></ul>') AS frag");
  expect_true(parsed_literal.query.has_value(), "parse PARSE() with string expression");

  auto parsed_subquery =
      markql::parse_query("SELECT li FROM PARSE(SELECT inner_html(div, 2) FROM document) AS frag");
...
void test_eval_like_wildcards() {
  ...
  auto percent =
      run_query(html, "SELECT div FROM document WHERE text LIKE '%c' ORDER BY node_id ASC");
  ...
  auto underscore =
      run_query(html, "SELECT div FROM document WHERE text LIKE 'a_c' ORDER BY node_id ASC");
...
void test_eval_string_functions_in_select() {
  ...
  if (result.rows.size() >= 2) {
    ...
    expect_true(
        result.rows[1].computed_fields.find("concat") == result.rows[1].computed_fields.end(),
...
void test_eval_position_found_and_not_found() {
  ...
  auto found = run_query(html, "SELECT div FROM document WHERE POSITION('world' IN TEXT(div)) = 7");
  expect_eq(found.rows.size(), 1, "position found");
  if (!found.rows.empty()) {
    expect_true(found.rows[0].text == "hello world", "position found row value");
  }
  auto missing =
      run_query(html, "SELECT div FROM document WHERE POSITION('world' IN TEXT(div)) = 0");
...
void test_eval_case_expression_select_and_null_else() {
  ...
  if (null_result.rows.size() >= 2) {
    expect_true(null_result.rows[0].computed_fields.find("maybe_match") ==
                    null_result.rows[0].computed_fields.end(),
                "case without else row1 null");
    expect_true(null_result.rows[1].computed_fields.find("maybe_match") ==
                    null_result.rows[1].computed_fields.end(),
...
void test_project_missing_match_null_and_coalesce() {
  ...
  if (result.rows.size() >= 2) {
    expect_true(
        result.rows[0].computed_fields.find("missing") == result.rows[0].computed_fields.end(),
...
void test_regression_canonical_project_query() {
  std::string html =
      "<ul>"
      "<li><h3>London</h3><span>2 stops</span><span>22 hr 5 min</span><span role='text'>258564 "
      "JPY</span></li>"
      "<li><h3>Milan</h3><span>1 stop</span><span>24 hr 40 min</span><span role='text'>287610 "
      "JPY</span></li>"
      "</ul>";
  auto result = run_query(html,
                          "SELECT li.node_id, PROJECT(li) AS ("
...
void register_string_sql_tests(std::vector<TestCase>& tests) {
  ...
  tests.push_back(
      {"parse_project_nested_string_functions", test_parse_project_nested_string_functions});
  ...
  tests.push_back(
      {"eval_direct_text_excludes_descendants", test_eval_direct_text_excludes_descendants});
  tests.push_back({"eval_case_expression_select_and_null_else",
                   test_eval_case_expression_select_and_null_else});
  tests.push_back(
      {"project_case_and_selector_positions", test_project_case_and_selector_positions});
  tests.push_back(
      {"project_missing_match_null_and_coalesce", test_project_missing_match_null_and_coalesce});
  tests.push_back(
      {"project_selector_scope_and_first_match", test_project_selector_scope_and_first_match});
  tests.push_back({"project_trailing_comma_execution", test_project_trailing_comma_execution});
  tests.push_back(
      {"project_top_level_comparison_expression", test_project_top_level_comparison_expression});

Feature: CTE and projection interaction
  Scenario: WITH CTE projected fields remain stable
    ...
    And the MarkQL query "WITH cards AS (SELECT section FROM document WHERE attributes.class CONTAINS 'card') SELECT c.node_id, c.tag FROM cards AS c ORDER BY node_id ASC LIMIT 2 TO JSON();"

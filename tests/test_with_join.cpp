...
void test_lateral_select_alias_star_equivalent_to_select_self() {
  std::string html = "<div id='root'><span>A</span><span>B</span></div>";
  auto legacy =
      run_query(html,
                "WITH rows AS (SELECT n.node_id AS row_id FROM doc AS n WHERE n.tag = 'div') "
  ...
  auto canonical =
      run_query(html,
                ...
                "ORDER BY node_id");
  expect_eq(canonical.rows.size(), legacy.rows.size(),
            "select alias.* and select self equivalent row count in lateral");
  if (canonical.rows.size() != legacy.rows.size()) return;
  for (size_t i = 0; i < canonical.rows.size(); ++i) {
    expect_true(canonical.rows[i].node_id == legacy.rows[i].node_id,
                "select alias.* and select self equivalent node_id");
  }
}
...
void register_with_join_tests(std::vector<TestCase>& tests) {
  ...
  tests.push_back({"lateral_select_alias_star_equivalent_to_select_self",
                   test_lateral_select_alias_star_equivalent_to_select_self});

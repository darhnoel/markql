...
void test_attr_shorthand_self_and_qualified_aliases() {
  ...
  auto implicit = run_query(html, "SELECT a FROM doc WHERE attr.id = 'x'");
  ...
  auto qualified = run_query(html, "SELECT a FROM doc AS n WHERE n.attr.id = 'x'");
...
void test_attr_shorthand_axis_paths() {
  std::string html =
      "<div id='root'><span id='child'></span></div><div id='skip'><span></span></div>";
  auto result = run_query(html, "SELECT span FROM doc WHERE parent.attr.id = 'root'");
...
void test_parse_alias_field_with_implicit_doc() {
  auto parsed = markql::parse_query("SELECT doc.node_id, TEXT(doc) FROM doc WHERE doc.tag = 'div'");
...
void test_parse_alias_field_with_explicit_alias() {
  auto parsed = markql::parse_query("SELECT n.node_id, TEXT(n) FROM doc AS n WHERE n.tag = 'div'");
...
void test_duplicate_source_alias_rejected() {
  auto parsed = markql::parse_query("SELECT n.node_id FROM doc AS n AS m WHERE n.tag = 'div'");
...
void test_doc_identifier_rejected_after_explicit_realias() {
  ...
  try {
    ...
    threw = true;
    expect_true(std::string(ex.what()) == "Identifier 'doc' is not bound; did you mean 'n'?",
